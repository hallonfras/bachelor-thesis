...
More recently efforts have been made in trying to infer a model based upon a running protocol implementation. This is done through a process called protocol state fuzzing. @protocol-state-fuzzing State fuzzing involves sending input sequences to a protocol implementation and constructing a model based on the responses.
...
In this work I seek to remedy this by developing TCP-Fuzzer, a modern implementation of state fuzzing for TCP. I then apply it to the TCP implementations of common operating systems in Windows, Linux and macOS.
